-- TODO: move lsp configuration to separate dir
return {
  { -- LSP Configuration & Plugins
    ...
    config = function()
      ...
      local already_installed = { 'gopls' }

      local ensure_installed = {}
      for k in pairs(servers or {}) do
        if not vim.tbl_contains(already_installed, k) then
      ...
      local goplsPath = 'gopls'
      if curBufPath:find 'arcadia' ~= nil then
        local openPop = assert(io.popen('ya tool gopls --print-path', 'r'))
        goplsPath = openPop:read '*all'
        openPop:close()
        goplsPath = vim.trim(goplsPath)
      end
